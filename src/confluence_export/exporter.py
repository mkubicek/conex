...
class Exporter:
    ...
    def _sweep_orphan_parks(self, run: _ExportRun) -> None:
        ...
        for child in run.output_dir.iterdir():
            ...
            if target.exists():
                # Target was claimed by another page during the aborted run
                # (typical of a half-finished swap). Preserve user data via
                # quarantine, then drop the stale manifest entry so Phase B
                # doesn't try to relocate the other page's content thinking
                # it belongs to this page id. Fresh export at the desired
                # path follows, with a warning telling the user where to
                # find the quarantined .workspace.
                if has_user_data:
                    self._quarantine_orphan(child, page_id, run.output_dir)
                else:
                    shutil.rmtree(child, ignore_errors=True)
                run.manifest.pop(page_id, None)
                continue
    ...
    def _export_single_page(
        ...
        *,
        ...
        # forever. Only remove files that look like our own exports (have
        # YAML frontmatter with a page_id key) so a manually-placed
        # notes.md sitting beside the page survives. Workspace and media
        # live in subdirs and are untouched.
        expected_md = md_path.name
        expected_html = md_path.with_suffix(".html").name
        stale_md_stems: set[str] = set()
        for stray in page_dir.glob("*.md"):
            if stray.name == expected_md:
                continue
            if not _looks_like_conex_export(stray):
                continue
            try:
                stray.unlink()
                stale_md_stems.add(stray.stem)
            except OSError:
                pass
        # Pair HTML cleanup to the .md cleanup we just did: only delete
        # <basename>.html when its sibling <basename>.md was stale conex
        # output. A user-authored manual.html with no matching conex .md
        # survives.
        for stray in page_dir.glob("*.html"):
            if self.debug and stray.name == expected_html:
                continue
            if stray.stem not in stale_md_stems:
                continue
            try:
                stray.unlink()
            except OSError:
...
def _looks_like_conex_export(path: Path) -> bool:
    """Heuristic: does this .md/.html file have conex-shaped YAML frontmatter?

    We only delete top-level files we generated, never user-placed notes.
    Reads at most the first 2 KiB and looks for an opening `---` plus a
    `page_id:` line — both come from convert_page's frontmatter and are
    very unlikely in a hand-written note.
    """
    try:
...
def _has_workspace_content(page_dir: Path) -> bool:
    """Return True if `page_dir` or any descendant has a non-empty .workspace.

    A parked subtree carries its children too, and each child has its own
    .workspace dir. A shallow check would miss user data nested inside a
    grandchild and rmtree the whole subtree on recovery.
    """
    if not page_dir.is_dir():
        return False
    try:
        for ws in page_dir.rglob(".workspace"):
            if not ws.is_dir():
                continue
            try:
                if any(ws.iterdir()):
